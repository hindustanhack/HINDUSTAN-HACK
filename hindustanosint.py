...
def display_banner():
    ...
    console.print(Panel(
        ...
        box=DOUBLE,
        border_style="red",
        title="[bold white]🔥 ULTIMATE NUMBER TO INFO v1.0 🔥[/bold white]",
        subtitle="[bold yellow]⚡ Developer: @Rolexseller1 | Channel: @Hindustanhack ⚡[/bold yellow]"
    ))
...
def print_info(num, data):
    ...
    console.print(f"[bold green]📊 Total Records :[/bold green] [bold white]{data.get('total_results', len(results))}[/bold white]")

    seen = set()
    idx = 0
    for rec in results:
        key = (rec.get("name"), rec.get("aadhar"), rec.get("mobile"))
        if key in seen:
            continue
        seen.add(key)
        idx += 1

        table = Table(show_header=False, box=ROUNDED, border_style="green", title=f"[bold yellow]📄 Result #{idx}[/bold yellow]")
        ...
        table.add_row("3 🔒 Aadhaar Number",    str(rec.get("aadhar", "N/A")))
        table.add_row("4 ✉  Registered Email",  str(rec.get("email", "N/A")))
        ...
        table.add_row("9 🆔 Reference ID",      f"REF-{num[-4:]}-{idx:02d}")

        console.print(table)

    console.print(Panel(
        "[bold magenta]👨‍💻 Developer :[/bold magenta] [bold white]ROLEX BHAI[/bold white]\n"
